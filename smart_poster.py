...
POST_MESSAGES = [
    "Я пользуюсь ботом @expat_rent_bot для проверки объявлений – за 5 секунд находит скрытые платежи и комиссии. Удобная штука, рекомендую.",
    "Для тех, кто ищет жильё в Европе: я пользуюсь @expat_rent_bot чтобы проверять объявления на скрытые платежи. Попробуйте – бесплатно 3 проверки.",
    "Столкнулись с Nebenkosten или непонятными комиссиями? Я пользуюсь @expat_rent_bot – разбирает объявление по косточкам.",
    "Переезжаете в Европу? Я пользуюсь @expat_rent_bot чтобы показать реальную стоимость аренды. Бесплатный тест – 3 проверки.",
]
